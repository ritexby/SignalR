...
// Каждая ступень размера текста по отдельности: планшет против наблюдения.
//
...
const kod = await (await fetch(BASE + "/api/admin/devices/enroll", {
  method: "POST", headers: SH, body: JSON.stringify({ name: "Планшет ступеней" }) })).json();
...
await plan.waitForTimeout(2500);
await nabl.goto(BASE + "/admin/#watch=" + encodeURIComponent("Планшет ступеней"));
