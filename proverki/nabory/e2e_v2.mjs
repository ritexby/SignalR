...
// ---- separate contexts so kiosk does NOT inherit the admin cookie ----
...
const enrollHidden = await kiosk.evaluate(() => document.getElementById('enroll').classList.contains('hidden'));
ok(enrollHidden, 'enrollment screen hidden after activation');
...
// ---- push slides to the GROUP → kiosk shows it ----
await aFetch('/playlist', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ target: 'group:' + grp.id, imageIds: [imageId], intervalSec: 3 }) });
await kiosk.waitForSelector('#slideA[src*="/media/"]', { timeout: 6000 });
ok(true, 'group target: kiosk shows slides');
...
// ---- second kiosk with no token → enrollment screen ----
...
await browser.close();
console.log(fail === 0 ? '\n✅ ALL v2 E2E CHECKS PASSED' : `\n❌ ${fail} v2 CHECK(S) FAILED`);
