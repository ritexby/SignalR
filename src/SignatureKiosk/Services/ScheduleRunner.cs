...
public class ScheduleRunner : BackgroundService
{
    ...
    public async Task<string> RunNow(ScheduleRule rule, CancellationToken cancel)
    {
        ...
        // Итог записывается у правила, чтобы оператор видел его и после перезагрузки страницы.
        // Раньше он приходил только ответом на нажатие: прочитал всплывашку и потерял, а «на
        // всех планшетах идёт подписание» это как раз то, что хочется перечитать.
        //
        // Дата суточного пуска НЕ ставится намеренно: пуск вручную не должен съедать сегодняшний
        // выход правила по времени. Поэтому пишется только когда и что вышло.
        _storage.MarkScheduleResult(rule.Id, "запущено вручную: " + result);
        return result;
    }
}
