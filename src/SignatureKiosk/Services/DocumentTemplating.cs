...
public static partial class DocumentTemplating
{
    ...
    private static bool Holds(VisibleWhen cond, IReadOnlyDictionary<string, string>? fields, bool своиИмена = false)
    {
        ...
        if (IsNumOp(cond.Op))
        {
            ...
            if (cond.Op == "numin")
            {
                // Пустой край означает «без предела», ровно как у промежутка дат и времени,
                // который считается WithinRange выше. Раньше числовой промежуток требовал обе
                // границы, и «5..» не выполнялся НИ ПРИ КАКОМ значении: оператор писал «от 5 и
                // выше», видел в редакторе описание промежутка и получал блок, которого клиент
                // не увидит никогда, а служба об этом молчала. Одна и та же запись «от..до»
                // значила у дат «без предела», а у чисел «никогда».
                var (от, до) = SplitRange(target);
                var естьНиз = TryNum(от, out var a);
                var естьВерх = TryNum(до, out var b2);
                // Оба края пусты: это не промежуток, а пустое место. Такое условие не
                // выполняется, иначе блок показался бы всем подряд по недописанному условию.
                if (!естьНиз && !естьВерх) return false;
                if (естьНиз && n < a) return false;
                if (естьВерх && n > b2) return false;
                return true;
            }
    ...
    private static void ПочиститьПравила(DocPage p)
    {
        ...
        foreach (var r in p.CheckRules ?? new List<CheckRule>())
        {
            ...
            var былоКлючей = r.Keys.Count(k => !string.IsNullOrWhiteSpace(k));
            r.Keys = r.Keys.Where(k => !string.IsNullOrWhiteSpace(k) && живые.Contains(k.Trim())).ToList();
            var какЗовётся = r.Kind == "minchecked" ? "«отметить не меньше N»" : "«взаимоисключающие»";
            // Правило про один пункт бессмысленно: взаимоисключать не с чем, а «не меньше N»
            // превращается в обычное «обязательный пункт». Но выбрасывать его молча нельзя:
            // оператор задал правило, увидел «сохранено» и уверен, что оно работает.
            if (r.Keys.Count < 2)
            {
                Срезано("правило " + какЗовётся + " убрано: в нём осталось меньше двух пунктов"
                        + (былоКлючей > r.Keys.Count ? " (часть перечисленных пунктов на странице нет)" : "")
                        + ". Правило про один пункт это просто обязательный пункт");
                continue;
            }
            // Требовать отметок больше, чем есть пунктов, значит запретить подписание навсегда.
            if (r.Kind == "minchecked")
            {
                var былоN = r.N;
                r.N = Math.Clamp(r.N <= 0 ? 1 : r.N, 1, r.Keys.Count);
                if (былоN != r.N)
                    Срезано("в правиле «отметить не меньше N» число уменьшено с " + былоN + " до " + r.N
                            + ": в перечне правила всего " + r.Keys.Count + " пункта. Иначе документ было бы "
                            + "не пройти вовсе");
            }
            правила.Add(r);
        }
        ...
        foreach (var r in правила.Where(x => x.Kind == "exclusive").ToList())
        {
            var обязательные = (p.Checkboxes ?? new List<DocCheckbox>())
                ...
                .ToList();
            // Хватает и ОДНОГО обязательного. Правило запрещает отметить больше одного, значит
            // обязательный пункт внутри него делает остальные варианты недостижимыми: клиент
            // выбирает «Отказываюсь», нажимает «Далее» и видит «Нужно отметить» над пунктом
            // «Согласен», которого он сознательно не выбирал. Замер: экран подписи не
            // открывался, пока клиент не отметит именно обязательный, то есть выбора у него не
            // было вовсе, хотя документ его предлагал.
            //
            // Раньше чинился только случай с двумя и более обязательными, а с одним оставался
            // непроходимый документ, и молча: ни ответ сохранения, ни проверка документа об
            // этом не говорили.
            if (обязательные.Count < 1) continue;
            ...
            Срезано(обязательные.Count > 1
                ? "взаимоисключающие пункты, каждый из которых обязателен, пройти нельзя: правило приведено к «выбрать ровно один»"
                : "во взаимоисключающем правиле пункт «" + Имя(PlainOf(обязательные[0].LabelRuns, обязательные[0].Label), обязательные[0].Key)
                  + "» был обязательным: остальные варианты клиент выбрать не смог бы. Правило приведено к «выбрать ровно один»");
        }
    ...
    private static void NormalizeScreens(DocumentConfig doc)
    {
        foreach (var p in doc.Pages)
        {
            ...
            if (kind is not ("signature" or "scan"))
            {
                ...
                if (HasInteraction(p) && !p.InPdf)
                {
                    // Само правило верное: страница, на которой клиент что-то подтверждает, из
                    // бумаги исчезать не должна. Но решение принимается за оператора, и молчать
                    // о нём нельзя: он остаётся уверен, что вступительный экран с согласием в
                    // бумагу не пойдёт.
                    Срезано("страница «" + Имя(PlainOf(p.HeadingRuns, p.Heading), "без заголовка")
                            + "» печатается в PDF: на ней клиент отмечает, выбирает, вписывает, "
                            + "подписывает или сканирует, и без неё бумага была бы неполной");
                    p.InPdf = true;
                }
                NormalizeOrder(p);
                continue;
            ...
            if (HasInteraction(p) && !p.InPdf)
            {
                // То же правило и тот же довод, что выше: решение принимается за оператора, и он
                // обязан о нём узнать.
                Срезано("страница «" + Имя(PlainOf(p.HeadingRuns, p.Heading), "без заголовка")
                        + "» печатается в PDF: на ней клиент отмечает, выбирает, вписывает, "
                        + "подписывает или сканирует, и без неё бумага была бы неполной");
    ...
    public static void Sanitize(DocumentConfig doc)
    {
        ...
        foreach (var p in doc.Pages)
        {
            ...
            // Правило, в котором осталось меньше двух пунктов, выбрасывается: взаимоисключать не
            // с чем, а «не меньше N» превращается в обычный обязательный пункт. Молчать об этом
            // нельзя: оператор задал правило, увидел «сохранено» и уверен, что оно работает.
            // Ключи могли отсеяться и потому, что таких пунктов на странице нет вовсе.
            var мало = p.CheckRules.Count(r => r.Keys.Count < 2);
            if (мало > 0)
                Срезано(мало == 1
                    ? "правило отметок убрано: в нём осталось меньше двух пунктов страницы"
                    : "правил отметок убрано " + мало + ": в каждом осталось меньше двух пунктов страницы");
            p.CheckRules = p.CheckRules.Where(r => r.Keys.Count >= 2).ToList();
            if (p.CheckRules.Count > 10)
            {
                Срезано("правил отметок на странице больше десяти: лишние убраны ("
                        + p.CheckRules.Count + " превращено в 10)");
                p.CheckRules = p.CheckRules.Take(10).ToList();
            }
            ...
            //
            // Сказать об этом ПочиститьПравила умеет сама и называет пункт по имени, поэтому
            // грубого счёта «стало обязательных меньше, чем было» тут больше нет: он говорил то
            // же самое второй раз и другими словами.
            ПочиститьПравила(p);
    ...
    private static void CleanBlock(DocBlock b)
    {
        ...
        if (b.Table is not null)
        {
            ...
            // Урезание называется вслух. Оператор вставлял таблицу на 50 строк, служба
            // отвечала «сохранено» без единого слова, а в документе оставалось 40: десять строк
            // исчезали, и узнать об этом можно было, только пересчитав их глазами. Механизм
            // предупреждений в соседних местах этого же файла работает, тут он просто не был
            // вызван.
            var всегоСтрок = (b.Table.Rows ?? new List<List<string>>()).Count(r => r is not null);
            var всегоСтолбцов = (b.Table.Rows ?? new List<List<string>>())
                .Where(r => r is not null).Select(r => r.Count).DefaultIfEmpty(0).Max();
            ...
            if (всегоСтрок > 40)
                Срезано("в таблице строк больше сорока: лишние убраны (" + всегоСтрок + " превращено в 40)");
            if (всегоСтолбцов > 8)
                Срезано("в таблице столбцов больше восьми: лишние убраны (" + всегоСтолбцов + " превращено в 8)");
