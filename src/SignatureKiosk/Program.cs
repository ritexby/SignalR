...
// Тело запроса, которое платформа не смогла разобрать, до обработчика не доходит вовсе, и наружу
// уходит голый 400: ни слова, ни заголовка, ни следа в журнале. Разобраться в этом снаружи нельзя
// ничем, кроме догадки, и это уже стоило нам разбирательства: документ с дробным порядком
// элемента («ord»: 12.5) отвергался молча, а выглядело это как «сохранение не работает».
// Здесь такой отказ получает причину по-русски и попадает в журнал.
// Отказ без единого слова. Тело, которое платформа не смогла разобрать, до обработчика не
// доходит вовсе: она сама отвечает 400 и не пишет ничего. Ни причины, ни следа в журнале, и
// разобраться в этом снаружи нельзя ничем, кроме догадки. Это уже стоило нам разбирательства:
// документ с дробным порядком элемента («ord»: 12.5) отвергался молча, а выглядело это как
// «сохранение перестало работать».
// Страницы состояний срабатывают ровно тогда, когда тело пустое, поэтому свои отказы с
// объяснением они не трогают.
app.UseStatusCodePages(async context =>
{
    var о = context.HttpContext.Response;
    if (о.StatusCode != StatusCodes.Status400BadRequest) return;
    var зпр = context.HttpContext.Request;
    var логи = context.HttpContext.RequestServices.GetService<EventLogService>();
    логи?.Add("warn", "api", "Запрос не разобран: " + зпр.Method + " " + зпр.Path, null);
    о.ContentType = "application/json; charset=utf-8";
    await о.WriteAsJsonAsync(new
    {
        error = "Тело запроса не разобрано. Скорее всего значение прислано не того вида.",
        подсказка = "Порядок элемента («ord») должен быть целым числом, признаки вида «required» "
                  + "булевыми, а тексты строками. Числа в кавычках и дробные порядки не принимаются."
    });
});
