...
(function () {
  ...
  /// Высоты кусков страницы, как их видит клиент.
  ///
  /// Общая разница высоты говорит только «расходится на столько-то», а чинить надо то место, где
  /// расходится. Поэтому планшет присылает список: короткое начало текста и высота. Наблюдение
  /// сверяет свой такой же список и называет первое несовпадение прямо в подсказке значка.
  /// Восемнадцати знаков хватает, чтобы узнать пункт, и мало, чтобы это было пересылкой документа.
  function рядыВысот(тело) {
    var узел = тело;
