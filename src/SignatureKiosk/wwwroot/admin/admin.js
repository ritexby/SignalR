...
(function () {
  ...
  function preparePasted(root) {
    ...
    // Чужое оформление снимается целиком.
    //
    // Word помечает своим шрифтом, кеглем в пунктах, цветом и заливкой каждый абзац, а иногда и
    // каждое слово. Всё это приезжало в документ как есть, и он переставал выглядеть так, как
    // задан в конструкторе: на планшете один заголовок оказывался крупнее другого без всякой на
    // то причины. Замер владельца, с которого это нашлось: внутренний кегль 26 у клиента против
    // 20 у оператора - относительный размер «крупнее», приехавший вставкой.
    //
    // Насыщенность и курсив остаются: это смысл, а не оформление. Заголовок, набранный в Word
    // жирным, жирным и останется. Всё остальное берётся из конструктора.
    root.querySelectorAll("*").forEach(function (n) {
      if (n.style) {
        ["font-size", "font-family", "color", "background-color", "background", "line-height",
         "letter-spacing", "text-transform"].forEach(function (имя) { n.style.removeProperty(имя); });
      }
      // Старая разметка Word помечает шрифт и цвет не стилем, а признаками самого тега.
      if (n.removeAttribute) { n.removeAttribute("face"); n.removeAttribute("size"); n.removeAttribute("color"); }
      // И относительный размер, если он приехал классом нашего же редактора из другого документа.
      if (n.classList) { n.classList.remove("rt-l"); n.classList.remove("rt-h"); }
    });
  ...
  function previewRuns(parent, runs) {
    (runs || []).forEach(function (r) {
      String(r && r.text != null ? r.text : "").split("\n").forEach(function (seg, i) {
        ...
        // Свой размер куска в пунктах. Множится на ступень, выбранную клиентом, ровно так же, как
        // это делает планшет (kiosk.js: calc(Npt * var(--doc-scale))). Без множителя такой кусок в
        // наблюдении не рос вовсе: при обычном размере числа совпадали, а на каждой ступени
        // расходились всё сильнее, и строка, которая у клиента переносилась, у оператора влезала.
        // Это и был весь остаток расхождения.
        //
        // Запасная единица в var() обязательна: тот же previewRuns рисует предпросмотр в
        // редакторе, где переменной сцены нет, и без запасного значения всё правило отбросилось
        // бы целиком, а кусок потерял бы свой размер.
        var pt = parseInt(r.sizePt, 10);
        if (pt >= 8 && pt <= 40) { span.className = ""; span.style.fontSize = "calc(" + pt + "pt * var(--wt-scale, 1))"; }
  ...
  // Пробная строка. Один в один такая же лежит в kiosk.js. Менять только в обоих файлах разом.
  var ПРОБНАЯ_СТРОКА = "Прием лекарственных средств, БАДов, гормональных препаратов, химиотерапии";
  var свояПроба = null;

  /// Ширина пробной строки в этом браузере, на этой системе, тем же шрифтом и кеглем, что мерил
  /// планшет. Зависит только от шрифта и от того, как система считает ширину букв, поэтому
  /// меряется один раз. Меряется внутри сцены: там свой шрифт и своя точность ширины букв.
  function ширинаСвоейПробы(сцена) {
    if (свояПроба !== null) return свояПроба;
    if (!сцена) return 0;
    ...
    п.style.cssText = "position:absolute;left:-9999px;top:0;visibility:hidden;white-space:nowrap;" +
                      "font-weight:700;font-size:20px;letter-spacing:normal";
    сцена.appendChild(п);
    // offsetWidth, а не рамка: сцена стоит под transform: scale, и рамка отдала бы сжатое число.
    // В первом заходе так и вышло - 636 вместо 768, и подгонка считалась от чужой величины.
    // Округление до целого здесь стоит меньше десятой доли точки на всю строку и роли не играет.
    var ш = п.offsetWidth;
    п.remove();
    // Запоминаем только когда шрифт документа точно загружен: до этого браузер рисует запасным, и
    // запомненное навсегда число было бы чужим. Именно так и вышло в первом заходе - 953 вместо
    // 780, доля -0,118em, поправка отброшена как несуразная, и подгонка не работала вовсе.
    if (document.fonts && document.fonts.check && !document.fonts.check("700 20px Roboto")) {
      ждатьШрифт();
      return ш;
    }
    свояПроба = ш;
  ...
  /// Дождаться шрифта и пересобрать сцену. Один раз: подписок много не нужно, а пересборка
  /// дешёвая, она меняет только числа в переменных сцены.
  var шрифтаЖдём = false;
  function ждатьШрифт() {
    if (шрифтаЖдём || !document.fonts || !document.fonts.ready) return;
    шрифтаЖдём = true;
  ...
  /// Догнать ширину букв планшета межбуквенным расстоянием.
  ///
  /// Кегли и ширина тела к этому месту уже совпадают: их планшет присылает числами. Остаётся то,
  ...
  /// оператора другое, разница около процента. Копится она по буквам, поэтому и лечится по
  /// буквам: добавка на букву равна разнице, делённой на число букв. Задаётся долей от кегля, а
  /// не точками, чтобы работать при любом размере текста, который выбрал клиент.
  ///
  /// Итог: строка, которая у клиента не влезла, не влезает и у оператора. Ради этого всё.
  function догнатьШиринуБукв(сцена, мерки) {
    var своя = ширинаСвоейПробы(сцена);
    var чужая = мерки && мерки.proba;
    var букв = (мерки && мерки.probaN) || ПРОБНАЯ_СТРОКА.length;
    var кегль = (мерки && мерки.probaKegl) || 20;
    // Оба числа кладутся на сцену признаком: разобраться, почему подгонка не встала, должно быть
    // можно, не открывая консоль. Первым идёт планшет, вторым сцена.
    сцена.setAttribute("data-proba", (чужая || 0) + " / " + (своя || 0));
    if (!(своя > 0) || !(чужая > 0) || !(букв > 0)) { сцена.style.removeProperty("--wt-track"); return 0; }
    var доля = (чужая - своя) / букв / кегль;
    сцена.setAttribute("data-proba", чужая + " / " + своя + " / " + доля.toFixed(5) + "em"
      + " / рост " + ((мерки && мерки.boost) || 1));
    // Предел намеренный: до пяти сотых кегля на букву это разница шрифтовых движков, а больше
    // означает, что расходится что-то другое, и подменять это раздвижкой букв нельзя. Тогда
    // расхождение остаётся и его называет значок в шапке.
    if (!(доля > -0.05 && доля < 0.05)) { сцена.style.removeProperty("--wt-track"); return 0; }
    сцена.style.setProperty("--wt-track", доля.toFixed(5) + "em");
    return доля;
  }
  ...
  function размерыТекста(сцена, ф) {
    ...
    if (н && есть(н.base)) {
      // Надбавка системы планшета. getComputedStyle её не показывает, поэтому планшет меряет её
      // сам (kiosk.js, ростТекста) и присылает числом. Без этого наблюдение рисовало бы тем
      // кеглем, который планшету назначен, а не тем, которым он на самом деле рисует.
      var рост = (н.boost > 0.5 && н.boost < 3) ? н.boost : 1;
      п("--wt-base", н.base * рост);
      п("--wt-text", (есть(н.text) ? н.text : зажать(16.32, 0.022, 20) * м) * рост);
      п("--wt-h1", (есть(н.h1) ? н.h1 : зажать(20.8, 0.032, 32) * м) * рост);
      п("--wt-h2", (есть(н.h2) ? н.h2 : зажать(18.4, 0.026, 25.6) * м) * рост);
      п("--wt-btn", (есть(н.btn) ? н.btn : зажать(16, 0.022, 19.2)) * рост);
    ...
    // неё попадает и всё, что среда планшета сделала с размером шрифта помимо нашей кнопки.
    var ступень = (н && н.base > 0) ? (н.base / 16) : м;
    сцена.style.setProperty("--wt-scale", (ступень > 0.2 && ступень < 6) ? ступень.toFixed(4) : "1");
    var доля = догнатьШиринуБукв(сцена, н);
    // Сводка для подсказки значка. Одно наведение мышью должно давать полный ответ на вопрос
    // «почему у оператора не то, что у клиента», без консоли и без переписки.
    var своиКегли = function (имя) { return Math.round(parseFloat(сцена.style.getPropertyValue(имя) || "0") * 10) / 10; };
    watch.сводка = н
      ? ("Кегли: у клиента " + (н.base || "?") + " и " + (н.text || "?")
         + ", здесь " + своиКегли("--wt-base") + " и " + своиКегли("--wt-text")
         + ". Ширина тела: " + (н.bodyW || "?") + " и " + (function () {
             // Меряем, а не вычисляем: вычисленное из отступа совпадёт само с собой при любой
             // поломке и покажет 752 против 752, даже если на деле там другое число.
             var т = document.querySelector(".wt-body");
             return т ? т.clientWidth : "?";
           })()
         + ". Проба букв: " + (н.proba || "?") + " и " + (ширинаСвоейПробы(сцена) || "?")
         + ", добавка " + (доля || 0).toFixed(5) + "em, рост " + (н.boost != null ? н.boost : "?") + ".")
      : "Планшет своих мерок не прислал: работает старая страница, размеры считаются по формулам.";
  }
  ...
  function watchSoloStart(искомое) {
    ...
    // Значка расхождения в шапке больше нет. Он был заведён, чтобы найти причины, по которым
    // экран оператора расходился с экраном клиента, и своё дело сделал: нашлись свой размер куска
    // в пунктах, относительный размер в заголовке группы, чужая надпись в поле подписи и высота
    // этого поля. Всё это закрыто, положения кусков выравниваются по планшету, и постоянная
    // жёлтая надпись оператору больше ни о чём не говорит.
    //
    // Сами числа никуда не делись: они пишутся признаками data-raznica, data-vyravnivanie и
    // data-proba на сцене. Оператору их не видно, а разобраться по ним можно в любой момент.
  ...
  function watchRender() {
    ...
    if (!(st && st.scroll)) сказатьПроРазницу(0, 0, 0, 0, null, null);
    повторитьПрокрутку(body, st && st.scroll);
  }

  /// Повторить положение прокрутки планшета в сцене наблюдения.
  /// Тот же список кусков, что шлёт планшет, но снятый со своей сцены. Правила спуска и обрезки
  /// повторены слово в слово (kiosk.js, рядыВысот), иначе списки нельзя сравнивать.
  /// Те самые куски страницы, узлами. Правила спуска повторены слово в слово из kiosk.js,
  /// иначе списки нельзя сопоставлять по порядку.
  function обходКусков(тело) {
    var узел = тело;
    ...
    for (var i = 0; i < узел.children.length && i < 40; i++) {
      ...
      из.push({ у: д, часть: false });
      if (из.length < 60 && д.children && д.children.length > 1 && д.children.length <= 6) {
        for (var j = 0; j < д.children.length; j++) из.push({ у: д.children[j], часть: true });
      }
    }
    return из;
  }

  function рядыСвоиВысот(тело) {
    var узел = тело;
    // Спуск по единственным детям: содержимое у обеих сторон лежит в обёртке, и сверять надо
    // именно её детей, а не саму обёртку.
    while (узел && узел.children && узел.children.length === 1) узел = узел.children[0];
    var из = [];
    if (!узел || !узел.children) return из;
    for (var i = 0; i < узел.children.length && i < 40; i++) {
      ...
      из.push({ t: (д.textContent || "").replace(/\s+/g, " ").trim().slice(0, 18),
                ...
                n: (д.textContent || "").length, fs: внКегль, y: Math.round(д.offsetTop) });
  ...
  /// Поставить куски туда, где они стоят у клиента.
  ///
  /// Кегли, ширина и текст к этому месту уже совпадают: планшет присылает их числами, и подсказка
  /// значка это подтверждает словами «при том же кегле, ширине и тексте». Расходится только
  /// перенос строк, и это разница шрифтовых движков: у клиента Android, у оператора настольный
  /// браузер. Выровнять её вёрсткой нельзя - пробовали и правилом переноса, и точной геометрией
  /// букв, и запретом автоувеличения.
  ///
  /// Поэтому выравнивается не перенос, а положение. Планшет присылает, на какой высоте стоит
  /// каждый кусок, наблюдение подгоняет верхний отступ так, чтобы кусок встал туда же. Текст при
  /// этом не режется, не сжимается и не прячется: меняется только пустое место между кусками.
  /// Внутри куска перенос может остаться на строку иным, и об этом говорит значок.
  /// Состояние выравнивания признаком на сцене: разобраться, почему оно не сработало, должно быть
  /// можно, не открывая консоль. Ровно так же выведены пробы букв.
  function сказатьПроВыравнивание(текст) {
    var сцена = watch.node ? watch.node.closest(".watch-screen") : document.querySelector(".watch-screen");
    if (сцена) сцена.setAttribute("data-vyravnivanie", текст);
  }

  /// Нуль это тоже начало, а не «нет значения»: без этого первый кусок на нулевой высоте ронял
  /// бы весь расчёт.
  function чужоeНачалоПусто(з) { return (z_число(з) ? з : 0); }
  function z_число(з) { return typeof з === "number" && isFinite(з); }

  function выровнятьПоПланшету(тело, чужие) {
    watch.остаток = 0; watch.сорвано = "";
    if (!тело) { сказатьПроВыравнивание("нет тела"); return 0; }
    if (!чужие || !чужие.length) { сказатьПроВыравнивание("планшет рядов не прислал"); return 0; }
    if (чужие[0] && чужие[0].cut) чужие = чужие.slice(1);   // признак обрезки, а не кусок
    // Только сами куски, без их частей. Части идут в списке вперемешку, и у кого их сколько,
    // зависит от того, как каждая сторона собрала своё дерево. Сопоставлять вперемешку значит
    // рано или поздно сравнить заголовок группы с подписью отметки, и тогда защита по имени
    // отключает выравнивание целиком. Так и вышло на первом же прогоне.
    var узлы = обходКусков(тело).filter(function (з) { return !з.часть; });
    чужие = чужие.filter(function (ч) { return !ч.p; });
    if (!узлы.length || !чужие.length) { сказатьПроВыравнивание("пусто: своих " + узлы.length + ", чужих " + чужие.length); return 0; }
    var своёНачало = узлы[0].у.offsetTop, чужоеНачало = чужие[0].y;
    if (чужоеНачало == null) { сказатьПроВыравнивание("планшет не прислал положений"); return 0; }
    var поправлено = 0;
    for (var i = 1; i < узлы.length && i < чужие.length; i++) {
      // Части куска не двигаем ни при каких условиях. Они стоят внутри своего куска и едут вместе
      // с ним, а отдельная поправка накладывалась бы поверх поправки куска. Именно так у владельца
      // заголовок группы наехал на кнопки ДА и НЕТ: часть получила сдвиг дважды.
      if (чужие[i].y == null) continue;
      // Тот ли это кусок. Списки строятся одинаковыми правилами, но если они где-то разойдутся,
      // поправка попадёт не туда и разрушит страницу. Тогда лучше не выравнивать вовсе и сказать
      // об этом, чем двигать вслепую.
      // Сравниваем без пробелов вовсе: это опознание куска, а не сверка текста. Пробел, лишний
      // с одной стороны, не повод бросать выравнивание - на этом оно и сорвалось в первом прогоне,
      // «Гормоны*Антибиотик» против «Гормоны *Антибиоти».
      var безПробелов = function (т) { return String(т || "").replace(/\s+/g, ""); };
      var своёИмя = безПробелов((узлы[i].у.textContent || "").slice(0, 24));
      var чужоеИмя = безПробелов(чужие[i].t);
      if (своёИмя.slice(0, 14) !== чужоеИмя.slice(0, 14)) {
        watch.сорвано = "«" + чужоеИмя + "» против «" + своёИмя + "»";
        сказатьПроВыравнивание("сорвано на " + i + ": " + watch.сорвано);
        снятьВыравнивание(узлы);
        return 0;
      }
      var надо = чужие[i].y - чужоeНачалоПусто(чужоеНачало);
      var есть = узлы[i].у.offsetTop - своёНачало;
      // Выше низа предыдущего куска не поднимаем ни при каких числах. Когда кусок у оператора
      // выше клиентского (перенос лёг иначе), выравнивание тянуло следующий вверх, и он налезал
      // на текст: прогон показал наезд на 51 точку. Текст важнее совпадения положений, поэтому
      // здесь останавливаемся, а остаток называем вслух.
      var предел = (узлы[i - 1].у.offsetTop + узлы[i - 1].у.offsetHeight) - своёНачало;
      var цель = Math.max(надо, предел);
      var сдвиг = цель - есть;
      if (Math.abs(сдвиг) < 1) continue;
      // Больше двухсот точек это не разница переносов, а что-то другое: не двигаем.
      if (Math.abs(сдвиг) > 200) continue;
      var базовый = parseFloat(getComputedStyle(узлы[i].у).marginTop) || 0;
      узлы[i].у.style.marginTop = (базовый + сдвиг) + "px";
      поправлено++;
    }
    // Остаток после выравнивания. Утверждать «пункты стоят на одном месте» и не показать, на
    // сколько именно они разошлись после правки, значит просить верить на слово.
    for (var k = 1; k < узлы.length && k < чужие.length; k++) {
      if (чужие[k].y == null) continue;
      var о = Math.abs((чужие[k].y - чужоеНачало) - (узлы[k].у.offsetTop - своёНачало));
      if (о > watch.остаток) watch.остаток = Math.round(о);
    }
    сказатьПроВыравнивание("поправлено " + поправлено + " из " + Math.min(узлы.length, чужие.length)
      + ", остаток " + watch.остаток);
    return поправлено;
  }

  /// Снять все поправки. Нужна, когда выравнивать нельзя: остатки прежних поправок на новой
  /// раскладке хуже, чем их отсутствие.
  function снятьВыравнивание(узлы) {
    (узлы || []).forEach(function (з) { if (з && з.у) з.у.style.removeProperty("margin-top"); });
  }


  /// Где именно расходится. Отдаёт список мест, а не готовую строку: по нему принимается и
  /// решение показывать значок, и его надпись.
  ///
  /// Решать по одной общей высоте нельзя, и это стоило отдельной версии: если один кусок стал
  /// выше на строку, а другой ниже на строку, сумма почти не меняется, и значок молчал, хотя
  /// куски у клиента и у оператора стоят по-разному. Владелец так и сказал: расхождение вижу,
  /// значка нет.
  function гдеРасходится(тело, чужие) {
    if (!тело || !чужие || !чужие.length) return [];
    var обрезка = (чужие[0] && чужие[0].cut) ? чужие[0].cut : 0;
    if (обрезка) чужие = чужие.slice(1);
    var свои = рядыСвоиВысот(тело);
    var из = [];
    var меньше = Math.min(чужие.length, свои.length);
    for (var i = 0; i < меньше; i++) {
      ...
      if (чужие[i].fs && свои[i].fs && Math.abs(чужие[i].fs - свои[i].fs) > 0.5)
        добавка += " при внутреннем кегле " + чужие[i].fs + " и " + свои[i].fs;
    ...
    if (обрезка) {
      из.push("страница длиннее сорока кусков (их " + обрезка + "), ниже сорокового положение не выравнивается");
    }
    return из;
  }

  /// Шапка и подвал. Сверять одно тело мало: расхождение может сидеть в них, а на теле не
  /// сказаться вовсе, и тогда значок молчит при видимой глазом разнице.
  function гдеРасходитсяРамка(мерки) {
    if (!мерки) return [];
    var из = [];
    var пара = function (имя, чужое, свой) {
      var у = document.querySelector(свой);
      if (!(чужое > 0) || !у) return;
      var своё = Math.round(у.offsetHeight);
      if (Math.abs(чужое - своё) > 2) из.push(имя + ": у клиента " + чужое + ", здесь " + своё);
    };
    пара("шапка", мерки.headH, ".wt-head");
    пара("подвал", мерки.footH, ".wt-foot");
    return из;
  }
  ...
  function сказатьПроРазницу(своёВсего, своёОкно, чужоеВсего, чужоеОкно, тело, чужиеРяды) {
    var сцена = document.querySelector(".watch-screen");
    if (!сцена) return;
    var поВысоте = (чужоеВсего > 0) ? Math.round(чужоеВсего - своёВсего) : 0;
    var поОкну = (чужоеОкно > 0) ? Math.round(чужоеОкно - своёОкно) : 0;
    var места = гдеРасходится(тело, чужиеРяды)
      .concat(гдеРасходитсяРамка(watch.state && watch.state.metrics));
    // Любое несовпадение куска важно само по себе, даже если общая высота сошлась: оператор
    // смотрит на пункты, а не на сумму. Пороги по сумме и окну оставлены на случай, когда
    // список кусков не пришёл вовсе.
    var важно = места.length > 0 || Math.abs(поВысоте) > 24 || Math.abs(поОкну) > 8;
    if (!важно) { сцена.removeAttribute("data-raznica"); return; }
    сцена.setAttribute("data-raznica", "Содержимое страницы у клиента " + чужоеВсего + " точек, здесь " + своёВсего + "."
      ...
      + (места.length ? (" Расходятся: " + места.slice(0, 6).join("; ")
          + (watch.сорвано
              ? (". Положение НЕ выравнивается: списки кусков разошлись, " + watch.сорвано
                 + ". Двигать вслепую нельзя, поэтому оставлено как есть.")
              : (". Положение кусков при этом выровнено по планшету, наибольший остаток "
                 + (watch.остаток || 0) + " точек: пункты стоят на одном месте, отличаться может "
                 + "только перенос внутри куска."))) : "")
      + (watch.сводка ? (" " + watch.сводка) : ""));
  }
  ...
  function повторитьПрокрутку(body, прокрутка) {
    ...
    requestAnimationFrame(function () {
      var ряды = (watch.state && watch.state.metrics && watch.state.metrics.rows) || null;
      выровнятьПоПланшету(body, ряды);
