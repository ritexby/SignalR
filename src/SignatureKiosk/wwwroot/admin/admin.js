...
(function () {
  ...
  /// «одно место», «два места», «пять мест».
  function словоМест(n) {
    var сто = n % 100, десять = n % 10;
    if (сто >= 11 && сто <= 14) return "мест";
    if (десять === 1) return "место";
    if (десять >= 2 && десять <= 4) return "места";
    return "мест";
  }
  ...
  function сказатьПроРазницу(своёВсего, своёОкно, чужоеВсего, чужоеОкно, тело, чужиеРяды) {
    ...
    знак.textContent = места.length
      ? ("у клиента иначе: " + места.length + " " + словоМест(места.length))
      : ("у клиента иначе: " + (поВысоте > 0 ? "+" : "") + поВысоте + " точек содержимого");
