...
(function () {
  ...
  function обходКусков(тело) {
    ...
    for (var i = 0; i < узел.children.length && i < 40; i++) {
      ...
      if (из.length < 60 && д.children && д.children.length > 1 && д.children.length <= 6) {
        for (var j = 0; j < д.children.length; j++) из.push(д.children[j]);
      }
    }
    return из;
  }
  ...
  function выровнятьПоПланшету(тело, чужие) {
    if (!тело || !чужие || !чужие.length) return 0;
    var узлы = обходКусков(тело);
    if (!узлы.length) return 0;
    if (чужие[0] && чужие[0].cut) чужие = чужие.slice(1);   // признак обрезки, а не кусок
    var своёНачало = узлы[0].offsetTop, чужоеНачало = чужие[0].y;
    if (чужоеНачало == null) return 0;
    var поправлено = 0;
    for (var i = 1; i < узлы.length && i < чужие.length; i++) {
      if (чужие[i].y == null) continue;
      var надо = чужие[i].y - чужоеНачало;
      var есть = узлы[i].offsetTop - своёНачало;
      var сдвиг = надо - есть;
      if (Math.abs(сдвиг) < 1) continue;
      // Больше двухсот точек это не разница переносов, а что-то другое: не двигаем, пусть
      // расхождение останется видно значком.
      if (Math.abs(сдвиг) > 200) continue;
      var базовый = parseFloat(getComputedStyle(узлы[i]).marginTop) || 0;
      узлы[i].style.marginTop = (базовый + сдвиг) + "px";
      поправлено++;
    }
    return поправлено;
  }
  ...
  function сказатьПроРазницу(своёВсего, своёОкно, чужоеВсего, чужоеОкно, тело, чужиеРяды) {
    ...
    знак.title = "Содержимое страницы у клиента " + чужоеВсего + " точек, здесь " + своёВсего + "."
      ...
      + (места.length ? (" Расходятся: " + места.slice(0, 6).join("; ")
          + ". Положение кусков при этом выровнено по планшету: пункты стоят на одном месте, "
          + "отличаться может только перенос внутри куска.") : "")
