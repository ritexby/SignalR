...
(function () {
  ...
  function docMetaDialog(текущий, копияИз) {
    ...
    if (свойЗаголовок) {
      имя.readOnly = true;
      имя.setAttribute("data-role", "docnamero");
      имя.title = "Имя документа это его заголовок. Он правится в поле «Заголовок документа».";
      c.appendChild(el("p", "sig-meta",
        "Переименовать документ можно в поле «Заголовок документа»: это одно и то же имя, " +
        "и его же видит подписант. Здесь оно только показано."));
      var кЗаголовку = iconBtn("pen", "Изменить заголовок документа", "btn-ghost");
    ...
    ok.addEventListener("click", function () {
      // Имя, которое только показано, назад не отправляется: сервер оставит его как есть, и
      // документ с заголовком отсюда переименовать нельзя вовсе.
      var тело = { code: код.value, name: имя.readOnly ? "" : имя.value };
      if (текущий) {
  ...
  function освежитьПослеПереименования(id) {
    ...
    return api("/document?id=" + encodeURIComponent(id)).then(function (r) {
      ...
      return r.json();
    }).then(function (свежий) {
      var заголовок = String((свежий && свежий.title) || "");
      if (state.doc && String(state.doc.title || "") !== заголовок) {
        state.doc.title = заголовок;
        if ($("docTitle")) $("docTitle").value = заголовок;
      }
      отразитьЗаголовокВСписке();
      освежитьСвоюЗакладку();
    ...
    },
    {
      ...
      строить: function (х) {
        ...
        х.appendChild(апиТаблица(["Поле ответа", "Что означает"], [
          ...
          ["`placed`", "куда встал каждый присланный пункт: «пункт «...» добавлен на страницу «...»». Пустой список, если пунктов не присылали"],
  ...
  var ОБЩЕЕ_ДЛЯ_ГРУППЫ = {
    ext: "Доступ по ключу в заголовке `X-Api-Key`. Ключ заводится на вкладке «API-ключи» и показывается там один раз при создании: не записали, заводите новый. Вошедший в админку администратор проходит и без ключа, поэтому примеры отсюда работают прямо из браузера.",
    admin: "Всё под `/api/admin/*` и требует куки входа. `КУКА` в образцах это значение куки `sk_admin`: её ставит `POST /api/admin/login` с паролем администратора, и её видно в средствах разработчика браузера. Скопированный дословно образец, в котором `КУКА` не заменена, отвечает `401` с пустым телом. Частота не ограничена: за этими путями уже стоит проверка куки.",
    планшет: "Доступ по токену планшета, он выдаётся при активации и живёт на самом планшете."
  };

  var API_ГРУППЫ = [
    { id: "ref-ext", заголовок: "Внешнее API: /api/ext", общие: "ext",
      note: "Ограничение 600 запросов в минуту с одного адреса." },
  ...
  var API_ПУТИ = [
    ...
    },
    {
      ...
      req: [["`name`", "строка", "новое имя. Нет поля, имя не меняется"],
            ...
            ["`workstationId`", "строка", "внутренний номер места (его отдаёт `GET /api/admin/workstations` в поле `id`, это не тот код, которым адресуется внешнее API). Пустая строка или `null` снимает планшет с места. Нет поля, место не меняется"]],
      res: [["`ok`", "`true`"]],
  ...
  // ни здесь.
  var watch = { deviceId: null, name: "", doc: null, mode: "", state: null, node: null, solo: false, опрос: null, ждём: null };
